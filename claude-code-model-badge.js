...
// per-build variable the badge needs (element-creator fn, session object, effort level,
// supported-effort-levels) is captured here rather than assumed to keep a fixed name.
// The model list and current selection are *not* captured — the badge re-derives them
// itself from `.claudeConfig`/`.modelSelection` (stable property names on the session
// object), since neither appears at this anchor and local var names inside the render
// function are the most volatile part of the bundle between builds.
...
const patchSource = (source) => {
  ...
  const patched = source.replace(ANCHOR, (_match, spacer, cssName, modeSelector, hFn, sessionVar, effortVar, levelsVar) => {
    ...
    return `${spacer}${badge}${modeSelector}`;
  });
