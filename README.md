...
# There is also a rate-limit badge, on a branch

`feat/usage-limits-badge` keeps the badge above and adds a second one to its left —
**`5h 42% · 7d 18%`**, the share of the 5-hour session window and the 7-day weekly window
you've burned, with a bar each, reset times on hover, amber at 80% and red at 95%. Same script,
same install.

```
https://github.com/TsKyrk/cc-vscode-statusbar/tree/feat/usage-limits-badge
```
